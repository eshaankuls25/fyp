...
class GridOption:
	def __init__(self, dataset_pathname, options):
		...
		if sys.platform != 'win32':
			self.svmtrain_pathname = os.path.join(dirname, './Classifiers/libsvm/svm-train')
			self.gnuplot_pathname = '/usr/bin/gnuplot'
		else:
			# example for windows
			self.svmtrain_pathname = os.path.join(dirname, r'.\Classifiers\libsvm\svm-train.exe')
